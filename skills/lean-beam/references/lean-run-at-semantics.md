...
```bash
lean-beam run-at "Foo.lean" <snapshot-from-update> 30 2 "tac1"
lean-beam run-at "Foo.lean" <snapshot-from-update> 30 2 "tac2"
...
```bash
root="$(lean-beam run-at-handle "Foo.lean" <snapshot-from-update> 30 2 "tac1")"
...
```bash
printf 'def tmpA : Nat := 1\n\n#check tmpA\n' | lean-beam run-at "Foo.lean" <snapshot-from-update> 30 0 --stdin
```

Correct workflows:

```bash
root="$(lean-beam run-at-handle "Foo.lean" <snapshot-from-update> 30 0 "def tmpA : Nat := 1")"
...
```bash
lean-beam run-at "Foo.lean" <snapshot-from-update> 18 0 "exact h"
...
lean-beam run-at "Foo.lean" <snapshot-from-update> 18 4 "exact h"
```
...
printf '  first | exact h1\n  | exact h2\n' | lean-beam run-at "Foo.lean" <snapshot-from-update> 18 0 --stdin

# or read the probe from a file
lean-beam run-at "Foo.lean" <snapshot-from-update> 18 0 --text-file probe.lean

# ANSI-C shell quoting also works when you do want to keep everything on one command line
lean-beam run-at "Foo.lean" <snapshot-from-update> 18 0 $'  first | exact h1\n  | exact h2'
```

...
- [docs/SYNC_AND_DIAGNOSTICS.md](SYNC_AND_DIAGNOSTICS.md) owns the exact sync, refresh, save,
  progress, diagnostics, readiness, and stale-snapshot contract
...
severity sum, save `path` and `snapshot` come from its nested sync result, and a decoded close-save
